...
while(1)
{
    ...
    if ($child_pid == 0) {
        ...
        while(1) {
            ...
            if (int(rand(10)) < 2) { # Approximately 20% chance to disconnect
                print "SERVER: Terminating connection with $client_address:$client_port\n";
                last;
            }

            usleep(500000); # Sleep for 0.5 second to slow down the loop
        }
        $client_socket->close();
